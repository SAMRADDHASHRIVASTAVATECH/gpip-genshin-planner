...
import { 
  ...
  Globe, MapPin, Star, RefreshCw, Download, WifiOff, Wifi, Clock
} from 'lucide-react';
...
export interface OwnedCharacter {
  id: string; name: string; level: number; ascension: number;
  talentNa: number; talentSkill: number; talentBurst: number;
  weaponName: string; weaponLevel: number;
}
export interface PlaceState {
  id: string; name: string; region: string; explored: number;
  totalWaypoints: number; waypointsUnlocked: number; totalOrbs: number;
  statues: { id: string; discovered: boolean }[];
}
export interface AccountSetup { ar: number; arExp: number; targetAR: number; }
export interface AcquisitionSetup {
  targetCharId: string; targetCharName: string;
  targetCharRegion: string; currentPrimos: number;
}
export interface GeneratedAcquisitionPlan {
  targetCharName: string; primosNeeded: number;
  primosRemaining: number; pullsNeeded: number;
  questRequirements: { name: string; description: string; met: boolean }[];
  basePlan: {
    executionOrder: {
      order: number; phase: string; action: string; impact: string;
    }[];
  };
}
...
// ---------------------------------------------------------------------------
export const KEYS = {
  ACQUISITION: 'acq', SETUP: 'setup',
  CHARACTERS: 'chars', PLACES: 'places', PARTY: 'party'
};

export const DEFAULT_PLACES: PlaceState[] = [
  {
    id: 'p1', name: 'Narukami Island', region: 'inazuma',
    explored: 45, totalWaypoints: 15, waypointsUnlocked: 10,
    totalOrbs: 50, statues: [{ id: 's1', discovered: true }]
  },
  {
    id: 'p2', name: 'Avidya Forest', region: 'sumeru',
    explored: 10, totalWaypoints: 20, waypointsUnlocked: 2,
    totalOrbs: 100, statues: [{ id: 's2', discovered: false }]
  },
  {
    id: 'p3', name: 'Court of Fontaine Region', region: 'fontaine',
    explored: 80, totalWaypoints: 25, waypointsUnlocked: 25,
    totalOrbs: 60, statues: [{ id: 's3', discovered: true }]
  },
];
...
export const REGION_META: Record<string, { name: string; arRequired: number }> = {
  ...
  natlan:    { name: 'Natlan',    arRequired: 45 },
};
...
export const getPlacesByRegion = (places: PlaceState[], region: string) =>
  places.filter(p => p.region.toLowerCase() === region.toLowerCase());

export const generateAcquisitionPlan = (
  acq: AcquisitionSetup,
  setup: AccountSetup,
  _chars: OwnedCharacter[],
  _party: string[],
  _places: PlaceState[]
  ...
  return {
    ...
    questRequirements: [
      {
        name:        `Unlock ${acq.targetCharRegion}`,
        description: `Reach AR and complete Archon Quest for ${acq.targetCharRegion}`,
        met:         setup.ar >= (REGION_META[acq.targetCharRegion.toLowerCase()]?.arRequired ?? 1),
      },
    ],
    basePlan: {
      executionOrder: [
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500 font-mono">
              Plan Primogems, Quests, and Progression for your Target
            </p>
          </div>
        </div>
        <button
          ...
          <div className="p-6 space-y-8 border-t border-surface-accent">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Target Character — populated from /api/characters */}
                <div className="space-y-1">
                  <label className="text-[10px] font-mono text-zinc-500 block">Target Character</label>
                  <select
                    className="w-full bg-zinc-900 border border-zinc-800 rounded-lg p-2.5 text-white text-sm"
                    value={acqSetup.targetCharName}
                    onChange={e => setAcqSetup(s => ({ ...s, targetCharName: e.target.value }))}
                  >
                    <option value="">
                      {dbChars.length === 0 ? '-- Fetch Game Data first --' : '-- Select Target --'}
                    </option>
                    {dbChars.map(c => (
                ...
                <div className="mt-4 p-4 bg-zinc-950 border border-zinc-800 rounded-xl">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[10px] font-mono text-zinc-500 ml-auto">
                      AR {targetRegionMeta.arRequired} required
                    </span>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                    {targetRegionPlaces.slice(0, 6).map(place => {
                      const undiscovered = place.statues.filter(s => !s.discovered).length;
                      return (
                        <div key={place.id} className="p-2 bg-zinc-900 rounded-lg border border-zinc-800">
                          ...
                          {undiscovered > 0 && (
                            <div className="text-[9px] font-mono text-orange-400 mt-0.5">
                              <Star size={8} className="inline mr-0.5" />{undiscovered} undiscovered
                            </div>
                          )}
                          {place.waypointsUnlocked < place.totalWaypoints && (
                            <div className="text-[9px] font-mono text-yellow-500/70 mt-0.5">
                              ...
                              {place.totalWaypoints - place.waypointsUnlocked} wp unopened
                            </div>
                          )}
                        </div>
                      );
            ...
            <div>
              ...
              <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-xl mb-4 space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  {/* Character dropdown — from /api/characters */}
                  <div className="md:col-span-2">
                    ...
                    >
                      <option value="">
                        {dbChars.length === 0 ? '-- Fetch Game Data first --' : '-- Select --'}
                      </option>
                      {dbChars.map(c => (
                        <option key={c.id} value={c.name}>{c.name}</option>
                      ))}
                  ...
                  {/* Level / Ascension */}
                  {([['Lv', 'level', 1, 90], ['Asc', 'ascension', 0, 6]] as const).map(
                    ([label, field, min, max]) => (
                      <div key={field}>
                ...
                <div className="grid grid-cols-3 md:grid-cols-6 gap-3">
                  {/* Talents */}
                  {([['NA', 'talentNa'], ['Skill', 'talentSkill'], ['Burst', 'talentBurst']] as const).map(
                    ([label, field]) => (
                      <div key={field}>
                  ...
                  {/* Weapon dropdown — from /api/weapons */}
                  <div className="md:col-span-2">
                    ...
                    >
                      <option value="">
                        {dbWeapons.length === 0 ? '-- Fetch Game Data first --' : '-- Select --'}
                      </option>
                      {dbWeapons.map(w => (
              ...
              {characters.length > 0 && (
                <div className="space-y-2">
                  <p className="text-[10px] font-mono text-zinc-500">
                    Click a character to add/remove from active party (max 4).
                  </p>
                      ...
                      return (
                        ...
                        >
                          <div className={`w-3 h-3 rounded-full border ${
                            inParty ? 'bg-purple-500 border-purple-500' : 'border-zinc-600'
                          }`} />
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-bold text-white truncate">{c.name}</div>
                            <div className="text-[10px] font-mono text-zinc-500">
                              Lv {c.level} · Asc {c.ascension} · Talents {c.talentNa}/{c.talentSkill}/{c.talentBurst} · Weap Lv {c.weaponLevel}
                            </div>
                          </div>
                          <button
      ...
      {plan && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { label: 'CURRENT PRIMOGEMS',  value: acqSetup.currentPrimos.toLocaleString(), color: 'text-white'  },
              { label: 'REQUIRED PRIMOGEMS', value: plan.primosNeeded.toLocaleString(),       color: 'text-purple-400', sub: 'For 160 Pulls Guarantee' },
              {
                label: 'REMAINING NEEDED',
                value: plan.primosRemaining.toLocaleString(),
                color: plan.primosRemaining === 0 ? 'text-green-400' : 'text-red-400',
                sub: `${plan.pullsNeeded} pulls short`,
              },
            ].map(stat => (
              <div key={stat.label} className="p-4 bg-surface border border-surface-accent rounded-xl text-center">
                ...
                {stat.sub && <div className="text-[10px] text-zinc-500 font-mono mt-1">{stat.sub}</div>}
              </div>
            ))}
          </div>
              ...
              {plan.questRequirements.map((req, i) => (
                <div
                  ...
                  className={`p-4 rounded-xl border flex items-start gap-3 ${
                    req.met ? 'bg-green-500/5 border-green-500/20' : 'bg-zinc-900 border-zinc-800'
                  }`}
                >
                      ...
                      <div className="text-[10px] font-mono space-y-0.5">
                        ...
                        {undiscovered > 0 && (
                          <div className="text-orange-400">
                            → {undiscovered} statue{undiscovered > 1 ? 's' : ''} undiscovered
                          </div>
                        )}
                        {orbsLeft > 0 && (
          ...
          <div className="p-6 bg-surface border border-surface-accent rounded-2xl">
            <h3 className="text-lg font-display text-white mb-4 flex items-center gap-2">
              ...
              Progression Route (Preparing for {plan.targetCharName})
            </h3>
            <div className="space-y-2">
              {plan.basePlan.executionOrder.map(step => (
                <div
                  key={step.order}
                  className={`p-3 border-l-2 rounded-r-xl flex items-start gap-3 ${ImpactColor[step.impact] ?? ImpactColor.Low}`}
                >
...
interface SyncResult {
  ...
  counts?: { characters: number; weapons: number; materials: number; regions: number; enemies: number; places?: number };
  message?: string;
  error?: string;
}
...
function SyncManager() {
  ...
  const fetchDbStatus = useCallback(async () => {
    try {
      ...
      setDbStatus({ seeded: true, charCount: 95, weapCount: 180, matCount: 320, lastSync: null });
    }
  }, []);
  ...
  const handleFetchAll = async () => {
    ...
    try {
      ...
      if (data.success && data.counts) {
        ...
        if (data.counts.places) addLog(`Places seeded: ${data.counts.places}`);
      ...
      addLog('Cloud-hosted version detected — local database import unavailable in browser mode.');
    } finally {
      setIsFetching(false);
    }
  };
    ...
    try {
      ...
      if (data.success) {
        ...
        if (ni.total > 0) {
          ...
          if (ni.characters > 0) addLog(`  + ${ni.characters} new characters`);
          if (ni.weapons    > 0) addLog(`  + ${ni.weapons} new weapons`);
          if (ni.materials  > 0) addLog(`  + ${ni.materials} new materials`);
        } else {
      ...
      addLog('Cloud-hosted version detected — local database refresh unavailable in browser mode.');
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto p-6">
      ...
      <div className={`p-4 rounded-xl border flex items-center gap-3 ${
        isSeeded ? 'bg-green-500/5 border-green-500/20' : 'bg-orange-500/10 border-orange-500/30'
      }`}>
        {isSeeded
          ? <>
              <WifiOff size={18} className="text-green-400" />
              <div>
                <p className="text-sm font-bold text-green-400">Offline Mode Active</p>
                <p className="text-xs text-zinc-400 font-mono">
                  Local database is seeded. All planner features are available offline.
                </p>
              </div>
            </>
          : <>
              ...
              <div>
                <p className="text-sm font-bold text-orange-400">Initial Setup Required</p>
                <p className="text-xs text-zinc-400 font-mono">
                  Click "Fetch Game Data" to import all Genshin data into your local offline database.
                </p>
              </div>
            </>
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-surface border border-surface-accent rounded-2xl space-y-4">
          ...
          <p className="text-xs text-zinc-400 font-mono leading-relaxed">
            Imports all characters, weapons, materials, enemies, and regions from the bundled
            internal Genshin dataset into your local SQLite database. Run this once on first launch.
          </p>
        ...
        <div className="p-6 bg-surface border border-surface-accent rounded-2xl space-y-4">
          ...
          <p className="text-xs text-zinc-400 font-mono leading-relaxed">
            Checks for newly added game content and updates the local database safely.
            User progression is never reset.
          </p>
          <ul className="text-xs text-zinc-500 font-mono space-y-1">
...
export default function UnifiedPlanner() {
  ...
  const renderContent = () => {
    switch (activeView) {
      case 'acquisition': return <AcquisitionPlanner />;
        ...
        return (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-4">
            ...
            <p className="text-zinc-400 font-mono text-sm max-w-md">
              Welcome to your Genshin Impact progression planner. Select a tool from the
              sidebar to begin planning your acquisitions or managing your local database.
            </p>
          </div>
        );
      default: return null;
